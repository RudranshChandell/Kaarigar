...
export default function ProviderDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [servicesRes, bookingsRes, reviewsRes, reportsRes, contactsRes, docsRes, ratingRes] = await Promise.all([
  ...
  const handleSubmitReport = async (reportForm) => {
    ...
    if (!targetType || !targetId || !normalizedReason) {
    ...
    try {
      await createReport(provider.id, targetType, normalizedTargetId, normalizedReason);
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top,_rgba(29,78,216,0.08),_transparent_28%),linear-gradient(180deg,_#fafcff_0%,_#eff6ff_100%)] text-slate-900">
      ...
      <header className="sticky top-0 z-40 border-b border-white/70 bg-white/85 backdrop-blur-xl">
        <div className="mx-auto flex w-full max-w-[1700px] items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
          <button type="button" onClick={() => setActiveTab("overview")} className="flex items-center gap-3 text-left">
            ...
            <span>
              <span className="block text-sm font-medium uppercase tracking-[0.24em] text-slate-500">Kaarigar</span>
      ...
      <main className="relative mx-auto w-full max-w-[1700px] px-4 py-6 sm:px-6 lg:px-8">
        ...
        {activeTab === "messages" && (
          <MessagesTab customers={chatContacts} selectedCustomer={selectedCustomer} setSelectedCustomer={setSelectedCustomer} token={token} />
        )}

        {activeTab === "reports" && (
          provider ? (
            <ReportsTab bookings={bookings} reports={reports} onSubmit={handleSubmitReport} />
          ) : (
...
function BookingsTab({ bookings, onStatusChange, onMarkComplete }) {
  ...
  const sortedBookings = useMemo(() => {
    ...
    if (sortBy === "dateAsc" || sortBy === "dateDesc") {
      list.sort((left, right) => {
        const leftDate = parseBookingDateTime(left)?.getTime() || 0;
        const rightDate = parseBookingDateTime(right)?.getTime() || 0;
        return sortBy === "dateAsc" ? leftDate - rightDate : rightDate - leftDate;
      });
      return list;
    }

    if (sortBy === "status") {
      list.sort((left, right) => getStatusWeight(left.status) - getStatusWeight(right.status));
      return list;
    }

    if (sortBy === "customer") {
      list.sort((left, right) => {
        const leftName = left.customer?.name || left.customer?.username || "";
        const rightName = right.customer?.name || right.customer?.username || "";
  ...
  const getBookingStatusTheme = (status) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="w-full text-sm text-slate-500 sm:w-auto">Manage accepted, pending, and completed jobs.</div>
        <label className="ml-auto block">
          <span className="mb-1 block text-xs font-medium uppercase tracking-[0.2em] text-slate-500">Sort by</span>
          <select value={sortBy} onChange={(event) => setSortBy(event.target.value)} className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        {sortedBookings.map((booking) => {
          ...
          return (
            <div key={booking.id} className={`rounded-[1.5rem] border p-5 shadow-[0_14px_40px_rgba(15,23,42,0.08)] ${statusTheme.card}`}>
              <div className="flex items-start justify-between gap-3">
                <div>
                ...
                <span className={`rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] ring-1 ${statusTheme.badge}`}>{booking.status || "PENDING"}</span>
              </div>
              <div className="mt-4 space-y-2 text-sm text-slate-600">
                <div className="flex items-center gap-2"><FiClock className={statusTheme.icon} /> {booking.bookingDate} · {booking.timeSlot}</div>
                <div className="flex items-center gap-2"><FiMapPin className={statusTheme.icon} /> {booking.service?.location || "Location unavailable"}</div>
              </div>
      ...
      <div className="w-full border-b border-slate-200 bg-[linear-gradient(180deg,_#f8fbff,_#eef4ff)] p-4 lg:w-[340px] lg:border-b-0 lg:border-r">
        ...
        <div className="mt-4 h-[60vh] space-y-2 overflow-y-auto pr-1">
          <input
            ...
            className="mb-3 w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700"
          />
          ...
          )) : <p className="mt-8 text-center text-sm text-slate-500">No matching customers</p>}
        </div>
      </div>
...
function ReportsTab({ bookings = [], reports = [], onSubmit }) {
  ...
  const bookedCustomers = useMemo(() => {
    ...
    safeBookings.forEach((booking) => {
      ...
      if (customer?.id && !customerMap.has(customer.id)) {
  ...
  const selectedTargetType = targetTypeOptions.find((option) => option.value === reportForm.targetType);
  const targetOptions = reportForm.targetType === "CUSTOMER" ? bookedCustomers : safeBookings;
  const selectedTarget = targetOptions.find((option) => String(option.id) === reportForm.targetId);

  const getTargetLabel = (option) => {
    if (reportForm.targetType === "CUSTOMER") {
      return option.name || option.fullName || option.email || "Customer";
    }

    const customerName = option.customer?.name || option.customer?.fullName || option.customer?.username || option.customer?.email || "Unknown customer";
    const serviceCategory = option.service?.category || "Booking";
    const serviceSubcategory = option.service?.subcategory;
    const workLabel = serviceSubcategory ? `${serviceCategory} (${serviceSubcategory})` : serviceCategory;
    const date = option.bookingDate ? new Date(option.bookingDate).toLocaleDateString() : "Unknown date";
    const time = option.timeSlot ? ` - ${option.timeSlot}` : "";
    const status = option.status || "PENDING";

    return `${date}${time} - ${customerName} - ${workLabel} - ${status}`;
  };

  const handleSubmit = async (event) => {
    event.preventDefault();
    const submitted = await onSubmit(reportForm);
    if (submitted) {
      setReportForm({ targetType: reportForm.targetType, targetId: "", reason: "" });
    }
  };
                  ...
                  <div className="absolute left-0 right-0 top-full z-40 mt-2 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl">
                    <div className="space-y-2">
                      {targetTypeOptions.map((option) => {
                        const isSelected = reportForm.targetType === option.value;
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() => {
                              setReportForm((current) => ({ ...current, targetType: option.value, targetId: "" }));
                    ...
                    {targetOptions && targetOptions.length > 0 ? (
                      <div className="space-y-2">
                        {targetOptions.map((option) => {
                          ...
                          const isSelected = reportForm.targetId === optionValue;
                          ...
                          return (
                            <button
                              ...
                              title={optionLabel}
                              onClick={() => {
                                setReportForm((current) => ({ ...current, targetId: optionValue }));
                                setTargetDropdownOpen(false);
                              }}
                              className={`flex w-full items-center justify-between gap-3 rounded-xl px-3 py-2.5 text-left text-sm transition ${
                                isSelected
                      ...
                      <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 px-3 py-4 text-sm text-slate-500">No targets available</div>
                    )}
                  </div>
                )}
              </div>
