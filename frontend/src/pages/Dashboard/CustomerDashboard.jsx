  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        const [sRes, contactsRes] = await Promise.all([
          getAllServices(),
          getChatContacts(),
        ]);
...
setServicesWithDistance(
  ...
  setProviderContacts(Array.isArray(contactsRes.data) ? contactsRes.data : []);
  ...
  const filteredProviderContacts = providerContacts
    .filter((provider) => (provider?.name || "").toLowerCase().includes(providerSearch.trim().toLowerCase()))
          ...
          <div className="flex min-h-[78vh] flex-col overflow-hidden rounded-[1.9rem] border border-white/80 bg-white/95 shadow-[0_24px_80px_rgba(15,23,42,0.12)] lg:flex-row">
            <div className="w-full border-b border-slate-200 bg-[linear-gradient(180deg,_#f8fbff,_#eef4ff)] p-4 lg:w-[340px] lg:border-b-0 lg:border-r">
              ...
              <div className="mt-4 h-[60vh] space-y-2 overflow-y-auto pr-1">
                ...
                />
                {filteredProviderContacts.length > 0 ? filteredProviderContacts.map((provider) => (
                  <button
                    key={provider.id}
                    type="button"
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className={`text-xs ${selectedProvider?.id === provider.id ? "text-white/75" : "text-slate-500"}`}>{provider.role || "PROVIDER"}</div>
                        </div>
                      </div>
                      <div className={`h-3 w-3 rounded-full ${selectedProvider?.id === provider.id ? "bg-emerald-300" : "bg-slate-300"}`} />
                    </div>
                  </button>
                )) : <p className="mt-8 text-center text-sm text-slate-500">No matching providers</p>}
              </div>
            </div>
