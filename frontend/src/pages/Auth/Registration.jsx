...
import UnifiedPageShell from "../../components/common/UnifiedPageShell";
...
export default function Registration() {
  ...
  const handleRegister = async (event) => {
    ...
    if (!fullname || !email || !password || !location) {
      setError(t("registration.errorLocation"));
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      setError(err.response?.data?.message || t("registration.errorGeneral"));
      setLoading(false);
    }
  };
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-[1.75rem] border border-white/20 bg-white/10 p-6 backdrop-blur-xl">
              ...
              <p className="mt-3 max-w-md text-sm leading-6 text-white/80">
                Join Kaarigar as a customer or provider and keep everything from registration to booking in one consistent flow.
              </p>
            </div>
    ...
    >
      <div className="relative z-20 mx-auto flex w-full max-w-2xl flex-col justify-center py-2">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-8 space-y-4">
          ...
          <div>
            <h1 className="text-4xl font-bold tracking-tight text-slate-900 sm:text-5xl">{t("registration.title")}</h1>
        ...
        >
          ...
          <div className="space-y-6 rounded-[1.75rem] border border-slate-200 bg-slate-50/80 p-5 sm:p-6">
            ...
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
              <Field label={t("registration.fullname")}>
                <TextInput
              ...
              <Field label={t("registration.password")}>
              ...
              <Field label={t("registration.role")}>
                <select
                  value={role}
                ...
                >
                  <option value="CUSTOMER">{t("registration.roleCustomer")}</option>
                  <option value="PROVIDER">{t("registration.roleProvider")}</option>
                </select>
              </Field>
            </div>

            <Field label={t("registration.location")}>
